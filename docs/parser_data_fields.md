...
**Архитектура сбора:**
Большинство парсеров делает два этапа:
...
Исключение — **CrowdGames**: обходит все страницы каталога издателя, поиск локальный.
...
## Crowd Games (crowdgames.ru)

Crowd Games — российский издатель настольных игр. Каталог фиксированный (~167 игр).

**Технология**: `GET /collection/igry-crowd-games?page=N` → HTML, пагинация через `data-collection-infinity`.  
**Поиск**: локальный — весь каталог кешируется в SQLite (TTL 4ч), фильтрация по названию в памяти.  
**Без геоблока.** Обогащения (`_enrich`) нет — страницы товаров не запрашиваются.

### Каталог (`/collection/igry-crowd-games`)

| Поле | Источник | Статус |
|------|----------|--------|
| Название | `alt="..."` на изображении карточки | ✅ |
| URL товара | `href="/collection/shop/product/..."` | ✅ |
| External ID | `data-product-id="..."` на форме | ✅ |
| Цена | `<span class="...price-cur...">` (текущая, со скидкой) | ✅ |
| Изображение (thumbnail) | `data-src="https://images.crowdgames.ru/..."` | ✅ |
| Наличие | CSS-класс `is-zero-count` отсутствует | 📦 `raw["in_stock"]` |
| Кол-во игроков | нет на странице каталога | ❌ |
| Возраст / время | нет на странице каталога | ❌ |
| Описание | нет на странице каталога | ❌ |
| Правила PDF | нет на странице каталога | ❌ |
| Рейтинг | нет | ❌ |

**Реальные данные (Галактический круиз):**
```
external_id="1571691625"
price=8890 руб. | raw["in_stock"]=False (предзаказ)
url="https://www.crowdgames.ru/collection/shop/product/galakticheskiy-kruiz"
```

> **Примечание по цене:** на странице каталога бывает две цены — зачёркнутая (старая) и текущая. Парсер всегда берёт текущую из `<span class="...price-cur...">`.
