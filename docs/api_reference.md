...
> **Nullable (✅):** поле может быть `null` если сайт не предоставил данные или товар не был обогащён (например, HobbyGames с геоблоком).
...
| `hobbygames` | HobbyGames | **Только с российского IP** | С зарубежных IP возвращает заглушку. Nullable-поля будут `null` |
