...
Этот файл — оперативный roadmap для `services/web-test`. Архитектурный
обзор и список фичей живёт в [CLAUDE.md](CLAUDE.md). Здесь только то,
что сделано недавно, что не доделано и что планируется дальше.
...
- **F4.1-extended (parsers DB explorer)** — реализованы 3 виджета из 9
  (Inventory, ProductsBrowser, Analytics с latency/top-queries/empty).
  Осталось: timeline, latency-histogram, store-distribution,
  parser-breakdown, raw-keys. Все endpoints на стороне parsers уже
  есть, нужны только React-компоненты с Recharts. После этого можно
  отключить `/dashboard` в parsers (vanilla JS) — заменён полностью.
