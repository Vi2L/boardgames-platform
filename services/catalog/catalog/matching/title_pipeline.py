...
_EDITION_RE = re.compile(
    ...
    # Одиночные слова «база»/«base» — только в контексте (в скобках или
    # на конце title), чтобы не зацепить случайные «база данных» в середине.
    # Кейсы из WB: «В поисках Эльдорадо (база)», «В поисках Эльдорадо база».
    r'\(\s*баз[аы]\s*\)|'           # «(база)» / «(базы)»
    r'\(\s*base\s*\)|'              # «(base)»
    r'\s+баз[аы]\s*$|'              # «… база» / «… базы» в конце title
    r'\s+base\s*$|'                  # «… base» в конце title
