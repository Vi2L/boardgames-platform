...
- **`match_log` retention (CAT-11, 2026-05-18)**: APScheduler-job `match_log_retention` (cron `0 2 * * *`, сид в миграции 0015) ежедневно дёргает `auditor.evict_older_than(days=Settings.match_log_retention_days)` (default 90). Удаляются только **завершённые** записи: `reverted_at IS NOT NULL OR action = 'revert'`. Активные auto-match'и (нереверченные) сохраняются независимо от возраста — они нужны для возможности будущего revert. Override через cron/params в `scheduler_configs.params.retention_days`. ImportJob (`type='match-log-retention'`) пишет в payload `{retention_days, deleted, error}` — UI sched-history показывает результат.
- **Tier 0 cache invalidation (CAT-12, 2026-05-18)**: оператор может инвалидировать decision в `match_decisions` через UI (Catalog → Журнал матчинга → кнопка `cache` в строке `reject`/`auto_t3`) или API:
  - `DELETE /matching/decisions/{title_norm}` — точечно.
...
- **Batch lookup для SearchPage группировки (WT-F11, 2026-05-18)**: `POST /matching/lookup-batch` — read-only batch резолв game_id через `match_sync` (T0 cache + T1 trgm). Принимает `{items: [{title, store_slug?}], include_related_offers: bool}`, возвращает `{matches[], games[]}`. `matches[idx]` = резолв для items[idx] (game_id может быть null = orphan). `games[]` — уникальные сматченные игры с `title/title_ru` + `related_offers` из `catalog.offers` (только `match_status in ('auto', 'manual')`). Это даёт UI «также есть в магазинах» даже если они не пришли в текущем SSE-поиске. Limit 200, read scope. Не пишет в БД и не enqueue'ит — чистый read-only. Использование: web-test SearchPage после завершения SSE дёргает endpoint, группирует свои results по `game_id` (точно), fallback на frontend-fuzzy при недоступности catalog.
- **Auto Recovery Runner (CAT-4.5, 2026-05-18)**: `catalog/matching/v2/auto_recovery.py:run_once()` — scheduler-job (interval 60с) читает `enabled=true` правила из `auto_recovery_rules` (миграция 0014), для каждого проверяет condition + дедуп → выполняет action. Condition types: `circuit_state` (Ollama-модель), `breaker_state` (parsers per-store breaker через HTTP к `parsers/api/debug/breakers`), `job_completed` (последний ImportJob нужного type). Action types: `re_enqueue_skipped` (filters: reason/store_slug), `trigger_job`. **Дедуп через `last_triggered_at` + `dedup_minutes`** (default 5) — проще edge detection, не теряет события при рестарте. Все action'ы идемпотентны. Зависимость: `Settings.parsers_base_url` (default `http://parsers:8001`); если задать пустую строку — breaker-conditions автоматически False. Миграция 0017 сидит `scheduler_configs` записью.
