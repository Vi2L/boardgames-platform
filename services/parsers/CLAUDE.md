...
- **Wildberries (2026-05-14)**: парсер через публичный JSON `search.wb.ru/exactmatch/ru/common/v5/search` — тот же, что дёргает фронт WB. Один HTTP-запрос → 100 items (без pagination). **Pluggable backend** через env `WB_BACKEND=httpx|curl-cffi` (default `curl-cffi` — Angie/WB агрессивно rate-limit'ит DC-IP, TLS-impersonation проходит чаще). Override на лету — query-параметр `?wb_backend=...` в `/api/debug/parse`. WB endpoint v8+ требует preset-routing через `catalog.wb.ru` (тот всегда 403 из Docker) — поэтому используем legacy v5 без preset-redirect. **Strict-фильтр subjectId=120** (2026-05-18): возвращаем ТОЛЬКО товары с `subjectId == 120` («Настольные игры»). Раньше работал soft twin-search — при недоборе limit'а добивал «общей выдачей», в catalog шёл мусор. Теперь: лучше вернуть 4 настолки, чем 5 (4 настолки + 1 кроссовки). Если сломается — `bin/probe_wb4.py` для диагностики. См. roadmap [PRS-5] про enrichment через `card.wb.ru/cards/v{N}/detail`.
- **WB retry-стратегия (2026-05-18, PRS-6)**: при HTTP 429 — **exponential backoff с jitter**, 4 попытки (`_WB_RETRY_MAX_ATTEMPTS`): `delay = 1.5 * 2**attempt + random(0, 0.5)` → 1.5/3.0/6.0с + jitter. Раньше был 1 retry × фикс 2с — пользователь часто видел `HTTP 429` в UI. Jitter критичен: при параллельных запросах фиксированный sleep синхронизировал retry-пачку, теперь они рассеиваются. TLS-impersonation профиль — `chrome131` (default), override через `WB_IMPERSONATE` env (если новый профиль выпадет из curl-cffi таблицы или Angie начнёт его палить). UA и `Sec-Ch-Ua` headers подняты до Chrome 131 синхронно с JA3 — иначе Angie ловит расхождение между «заявленным» и реальным fingerprint. См. [PRS-6.1] в roadmap про L2 fallback (camoufox), если базового exp-backoff + breaker недостаточно.
- **Circuit Breaker per-store (2026-05-18, PRS-7)**: `parsers/utils/breaker.py:CircuitBreaker` — sliding-window failure rate breaker, общий шаблон для wb/ozon/avito. Используется через `get_breaker(slug)` singleton-registry. Default: `failure_threshold=0.5`, `window_sec=60`, `open_for_sec=300`, `min_samples=5` — после ≥50% failures в окне 60с (минимум 5 событий) цепь открывается на 5 минут. В `search()` каждого парсера: `is_available()` → fail-fast если open, иначе `record_success()`/`record_failure()` после запроса. Lazy half-open probe «по запросу» (без отдельного scheduler-job'а). Per-process state — multi-worker деплой каждый держит свой breaker, координация не нужна (цель — погасить шум на 5 минут). Diagnostics: `GET /api/debug/breakers` отдаёт snapshot всех активированных breaker'ов. Тесты через monkeypatch на `time.monotonic` (без real sleep).
...
| `/api/debug/breakers` | Snapshot Circuit Breaker'ов per-store (PRS-7) |
